              ...
              <div className="space-y-2 p-4 bg-white/5 rounded-xl border border-dashed border-white/10">
                ...
                {pollOptions.map((option, index) => (
                  <div key={index} className="flex items-center gap-2">
                    ...
                    <input type="text" required placeholder={`Inserisci l'opzione ${index + 1}...`} value={option} onChange={(e) => handleOptionChange(index, e.target.value)} className={`flex-1 px-3 py-1.5 rounded-lg border text-xs focus:outline-none focus:border-indigo-500 ${isDarkMode ? 'bg-slate-900 border-white/5 text-white' : 'bg-slate-50 border-black/5 text-slate-900'}`} />
        ...
        {!loading && posts.map((post) => {
          ...
          return (
            <div key={post.id} className={`rounded-2xl border backdrop-blur-md shadow-lg p-6 relative ${isDarkMode ? 'bg-white/5 border-white/10' : 'bg-white/60 border-black/5'}`}>
              ...
              {/* INTEGRATO: Intestazione Istituzionale con Logo di Cloudinary */}
              <div className="flex items-center space-x-3.5 mb-4 border-b border-white/5 pb-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-[10px] text-slate-400 font-light mt-0.5 uppercase tracking-wider">Comunicazione ufficiale dell'ente</p>
                </div>
              </div>
