...
export default function AssociationEvents({ associationId, associationLogoUrl, isAdmin, userId, isDarkMode }) {
  ...
  const fetchEvents = async () => {
    try {
      ...
      setError(err.response?.data?.message || 'Unable to load the events calendar.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleEventFileChange = async (e) => {
    ...
    try {
      ...
      alert(err.response?.data?.message || 'Error occurred during cover image upload.');
    } finally {
  ...
  const handleCreateEvent = async (e) => {
    ...
    try {
      ...
      alert(err.response?.data?.message || 'Error occurred during event creation.');
    } finally {
  ...
  const handleBookEvent = async (eventId) => {
    ...
    try {
      ...
      alert(response.data.message || 'Reservation confirmed!');
      fetchEvents();
    } catch (err) {
      alert(err.response?.data?.message || 'Unable to complete the reservation.');
    } finally {
  ...
  const handleShareEventToFeed = async (event) => {
    ...
    const payload = {
      ...
      type: 'EVENT',
      title: `Featured Event: ${event.title}`,
      contentBody: event.description || `Join our next meeting at ${event.location}.`,
    ...
    try {
      ...
      alert('Event successfully advertised on the board!');
    } catch (err) {
      alert(err.response?.data?.message || 'Unable to share the event.');
    }
  };
  ...
  const formatDateTime = (dateString) => {
    ...
    return `${parsedDate.toLocaleDateString('en-US', dateOpts)} at ${parsedDate.toLocaleTimeString('en-US', timeOpts)}`;
  };
  ...
  return (
    <div className="space-y-8 max-w-3xl mx-auto">
      {isAdmin && (
        <div className={`p-6 rounded-2xl border backdrop-blur-md shadow-xl transition-all ${isDarkMode ? 'bg-white/5 border-white/10' : 'bg-white/80 border-black/5'}`}>
          <h3 className={`text-sm font-semibold uppercase tracking-wider mb-4 ${isDarkMode ? 'text-indigo-400' : 'text-indigo-600'}`}>
            Schedule a New Assembly or Public Event
          </h3>
          <form onSubmit={handleCreateEvent} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-[10px] font-semibold text-slate-400 uppercase tracking-widest mb-1.5">Event Title</label>
                <input type="text" required placeholder="e.g., Extraordinary Budget Meeting" value={title} onChange={(e) => setTitle(e.target.value)} className={`w-full px-4 py-2 rounded-xl border text-sm focus:outline-none focus:border-indigo-500 ${isDarkMode ? 'bg-white/5 border-white/10 text-white' : 'bg-white/5 border-black/10 text-slate-900'}`} />
              </div>
              <div>
                <label className="block text-[10px] font-semibold text-slate-400 uppercase tracking-widest mb-1.5">Location or Venue</label>
                <input type="text" required placeholder="e.g., Main Headquarters or Zoom Link" value={location} onChange={(e) => setLocation(e.target.value)} className={`w-full px-4 py-2 rounded-xl border text-sm focus:outline-none focus:border-indigo-500 ${isDarkMode ? 'bg-white/5 border-white/10 text-white' : 'bg-white/5 border-black/10 text-slate-900'}`} />
              </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-[10px] font-semibold text-slate-400 uppercase tracking-widest mb-1.5">Date and Time</label>
              ...
              <div>
                <label className="block text-[10px] font-semibold text-slate-400 uppercase tracking-widest mb-1.5">Maximum Available Seats</label>
                <input type="number" required min="1" placeholder="e.g., 50" value={maxSlots} onChange={(e) => setMaxSlots(e.target.value)} className={`w-full px-4 py-2 rounded-xl border text-sm focus:outline-none focus:border-indigo-500 ${isDarkMode ? 'bg-white/5 border-white/10 text-white' : 'bg-white/5 border-black/10 text-slate-900'}`} />
              </div>
            </div>

            <div>
              <label className="block text-[10px] font-semibold text-slate-400 uppercase tracking-widest mb-1.5">Event Cover Image (Upload from device)</label>
              ...
              {uploadingImage && <p className="text-[11px] text-indigo-400 animate-pulse mt-1.5">Uploading image to cloud...</p>}
              {imageUrl && (
                <div className="mt-3 relative w-32 h-20 rounded-lg overflow-hidden border border-white/10">
                  ...
                  <button type="button" onClick={() => setImageUrl('')} className="absolute top-1 right-1 bg-black/70 text-white text-[9px] px-1.5 py-0.5 rounded uppercase font-mono">Remove</button>
                </div>
              )}
            </div>

            <div>
              <label className="block text-[10px] font-semibold text-slate-400 uppercase tracking-widest mb-1.5">Agenda Details / Notes</label>
              <textarea rows="3" placeholder="Describe the key points..." value={description} onChange={(e) => setDescription(e.target.value)} className={`w-full px-4 py-2 rounded-xl border text-sm resize-none focus:outline-none focus:border-indigo-500 ${isDarkMode ? 'bg-white/5 border-white/10 text-white' : 'bg-white/5 border-black/10 text-slate-900'}`} />
            </div>
      ...
      <div className="space-y-6">
        ...
        {loading && <p className="text-sm text-slate-400 animate-pulse italic">Updating calendar...</p>}
        {error && <p className="text-sm text-red-400 font-medium">{error}</p>}
        {!loading && events.length === 0 && (
          <p className="text-sm text-slate-500 italic py-8 text-center">No events currently scheduled.</p>
        )}
          ...
          return (
            <div key={event.id} className={`p-6 rounded-2xl border backdrop-blur-md shadow-lg flex flex-col md:flex-row md:items-center justify-between gap-6 transition-all duration-300 ${event.cancelled ? 'opacity-40 bg-red-500/5 border-red-500/20' : isDarkMode ? 'bg-white/5 border-white/10' : 'bg-white/60 border-black/5'}`}>
              <div className="space-y-2 flex-1">
                ...
                {/* EVENT BRAND HEADER WITH LOGO CLOUDINARY */}
                <div className="flex items-center space-x-3.5 mb-3 border-b border-white/5 pb-2.5">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center space-x-2">
                      ...
                      {event.cancelled && <span className="text-[9px] px-2 py-0.5 rounded-md font-mono bg-red-500/20 text-red-400 border border-red-500/30 font-bold uppercase tracking-wider">Cancelled</span>}
                    </div>
                  </div>
                </div>
                ...
                <p className="text-xs text-slate-400">Location: <span className={`font-medium ${isDarkMode ? 'text-slate-200' : 'text-slate-700'}`}>{event.location}</span></p>
              ...
              <div className="flex flex-col items-start md:items-end justify-between min-w-[180px] pt-4 md:pt-0 border-t md:border-t-0 md:border-l border-white/5 md:pl-6 gap-3">
                <div className="text-left md:text-right">
                  <p className={`text-xs font-semibold ${isFull ? 'text-red-400' : isDarkMode ? 'text-emerald-400' : 'text-emerald-600'}`}>{isFull ? 'Fully Booked' : `${availableSlots} seats left`}</p>
                  <p className="text-[10px] text-slate-500 font-mono mt-0.5">Total: {event.bookedSlots} / {event.maxSlots} occupied</p>
                </div>
                ...
                <div className="flex flex-col w-full gap-2 mt-2">
                  ...
                  {!event.cancelled && (
                    <button onClick={() => handleBookEvent(event.id)} disabled={isFull || bookingLoadingId === event.id} className={`w-full px-4 py-2 rounded-xl text-xs font-semibold uppercase tracking-wider text-center transition active:scale-95 cursor-pointer ${isFull ? 'bg-slate-800 text-slate-500 border border-slate-700 cursor-not-allowed opacity-50' : 'bg-indigo-600 hover:bg-indigo-700 text-white shadow-md disabled:opacity-50'}`}>
                      {bookingLoadingId === event.id ? 'Processing...' : 'Book Seat'}
                    </button>
                  )}
                </div>
              </div>
